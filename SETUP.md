...
> 最快的做法见「6. 手机 PWA 全屏」里的**一键配置链接**：
> 电脑上打开设置 → 点「生成手机配置链接」→ 复制发到手机 → 点开即自动配好。
...
所以做成链接：电脑上打开设置 → 同步区块 → 点 **「生成手机配置链接」** → **复制** →
发到微信/备忘录 → 手机点开，URL 和 Key 自动填好并开启同步，随后地址栏参数自动清除。
