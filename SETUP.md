...
> **改前端前必做**：把 `index.html` 的 `APP_VERSION` 和 `sw.js` 的 `CACHE` 同时 +1（v8 → v9）。
> 两个值必须一致，否则老用户可能一直跑旧壳。改完可以用仓库里的 `_check.mjs` 自检：
> `node _check.mjs`（会校验语法、DOM id 配对、同步逻辑回归、版本一致性）。
...
设置 → 云同步 → **「立即检查并同步」**，它会把下面这些逐条列出来，直接截图就能定位问题：

- **版本**：当前页面版本号（跟 `sw.js` 里的 `CACHE`、`index.html` 里的 `APP_VERSION` 必须一致；手机显示的不是最新版 = 页面是旧的）
- **同步开关 / Supabase 地址 / Anon Key 长度**
- **API Key**：有没有、几位（同步成功后应自动从云端取回）
- **读取云端**：HTTP 状态码（401 = 密钥错，404 = 地址错 / 表没建）
- **新旧对比**：该不该拉取
...
| 症状 | 原因 | 现在的处理 |
| --- | --- | --- |
| 粘贴的链接**前后带文字**（"看这个 … 手机上打开"） | 尾巴上的中文被当成 key 的一部分 → 存进去的是错密钥 → 401 | 先抠出链接本身 + 用 JWT 形状还原 key |
| 手填 URL 少写 `https://`、多带 `/rest/v1` 或结尾斜杠 | 拼出来的请求地址 404 | 统一清洗后再用 |
| 改完前端、手机上却没变 | GitHub Pages 给 HTML 打 `max-age=600` | SW 用 `cache: no-store` 拉页面 + 启动时主动查更新 |
| 手机放久了不同步 | 轮询被系统冻结 | 切回前台立刻同步一次 |
| 开着很久的旧页面把云端新数据覆盖了 | 整份 state 覆盖写 | 上传前先比对云端时间戳，云端更新则先拉再传 |
